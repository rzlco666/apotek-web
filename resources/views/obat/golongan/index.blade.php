        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-input">
                        <input type="hidden" name="kategori_obat_id" id="golongan-id">
                        <div class="form-group">
                            <label>{{ __('Nama Golongan Obat') }} <span class="text-danger">*</span></label>
                            <input type="text" name="nama_golongan" id="nama-golongan" class="form-control" required>
                        </div>
                    </form>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#form-modal').on('hidden.bs.modal', function() {
                $('#golongan-id').val('')
                $('#modal-form-label').text('Add Golongan')
                $('#nama-golongan').val('')
            ...
            $(document).on('click', '.edit-btn', function() {
                let id = $(this).data('id')
                let url = '{{ route('golongan-detail', ':id') }}'
                url = url.replace(':id', id)
                $('#golongan-id').val(id)
                $('#modal-form-label').text('Edit {{ __('Golongan') }}')

                $.get(url, function(response) {
                    let data = response.data
                    $('#nama-golongan').val(data.nama_golongan)
            ...
            $(document).on('click', '.detail-btn', function() {
                ...
                $.get(url, function(response) {
                    let data = response.data
                    $('#detail-golongan').html(data.nama_golongan)
            ...
            $('#save-btn').on('click', function() {
                ...
                let url = '{{ route('golongan-create') }}'
                let kategori_obat_id = $('#golongan-id').val()

                if (kategori_obat_id !== '') {
                    url = '{{ route('golongan-update', ':id') }}'
            ...
            $(document).on('click', '.delete-btn', function() {
                let id = $(this).data('id')
                let url = '{{ route('golongan-delete', ':id') }}'
                url = url.replace(':id', id)
